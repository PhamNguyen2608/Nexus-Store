...
if (
  location.pathname != "/nexus-store/" &&
  location.pathname != "/nexus-store/browse" &&
  selectedGame === false
) {
...
const handleSearchSubmit = (e) => {
  ...
  if (location.pathname != "/nexus-store/browse") {
    navigate("/nexus-store/browse");
  }
}
...
const handleSelectGame = (e) => {
  if (e.target.tagName === "BUTTON") {
    ...
    if (Number.isInteger(index) && index >= 0 && index < games.length) {
      ...
      navigate(`/nexus-store/games/${games[index]?.surname}`);
    }
  }
};
...
const openGamePage = (e) => {
  ...
  navigate(`/nexus-store/games/${selectedGameSurname}`);
}
...
useEffect(() => {
  ...
  if (location.pathname === "/nexus-store/") {
    setBrowsing(false);
  } else {
    setBrowsing(true);
  }
  ...
  } else if (location.pathname === "/nexus-store/browse") {
